...
<div style="max-width:900px;">
    ...
    <div style="display:flex; align-items:center; gap:16px; margin-bottom:2.5rem;">
        ...
        <div>
            <h1 style="font-size:32px; font-weight:800; letter-spacing:-0.04em;">Edit Karya</h1>
            <p style="color:rgba(255,255,255,0.4); font-size:14px;">Perbarui detail dan filosofi dari <span style="color:#fff;">{{ $project->title }}</span>.</p>
        </div>
    </div>
        ...
        <div style="display:grid; grid-template-columns: 1fr 300px; gap:24px; align-items:start;">
            ...
            <div style="display:flex; flex-direction:column; gap:24px;">
                <div class="glass-card" style="padding:32px;">
                    <div style="margin-bottom:24px;">
                        ...
                        <input type="text" name="title" class="form-input" value="{{ old('title', $project->title) }}" placeholder="e.g. Cybernetic Interface 2.0" required autofocus>
                    </div>

                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-bottom:24px;">
                        <div>
                            <label class="form-label">Kategori <span style="color:#f87171;">*</span></label>
                            <input type="text" name="category" class="form-input" value="{{ old('category', $project->category) }}" placeholder="e.g. Digital Architecture" required>
                        </div>
                        <div>
                            <label class="form-label">Tahun <span style="color:#f87171;">*</span></label>
                            <input type="text" name="year" class="form-input" value="{{ old('year', $project->year) }}" placeholder="2024" maxlength="4" required>
                        </div>
                    </div>

                    <div>
                        <label class="form-label">Deskripsi Naratif <span style="color:#f87171;">*</span></label>
                        <textarea name="description" class="form-input" rows="6" placeholder="Ceritakan filosofi dan tantangan teknis di balik project ini..." required style="resize:vertical; line-height:1.6;">{{ old('description', $project->description) }}</textarea>
                    </div>
                </div>
                ...
                <div class="glass-card" style="padding:32px;">
                    ...
                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-bottom:24px;">
                        <div>
                            ...
                            <input type="text" name="tech_stack" class="form-input" value="{{ old('tech_stack', is_array($project->tech_stack) ? implode(', ', $project->tech_stack) : $project->tech_stack) }}" placeholder="Laravel, Three.js, GSAP">
                        ...
                        <div>
                            ...
                            <input type="text" name="tags" class="form-input" value="{{ old('tags', is_array($project->tags) ? implode(', ', $project->tags) : $project->tags) }}" placeholder="immersive, high-perf, minimal">
                            <small style="color:rgba(255,255,255,0.2); font-size:10px; display:block; margin-top:6px;">Gunakan koma sebagai pemisah</small>
                        </div>
                    </div>
                    ...
                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                        <div>
                            ...
                            <input type="url" name="link_repo" class="form-input" value="{{ old('link_repo', $project->link_repo) }}" placeholder="https://github.com/...">
                        </div>
                        <div>
                            <label class="form-label">Link Demo / Live</label>
                            <input type="url" name="link_demo" class="form-input" value="{{ old('link_demo', $project->link_demo) }}" placeholder="https://interface.la/...">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div style="display:flex; flex-direction:column; gap:24px;">
                <div class="glass-card" style="padding:24px;">
                    ...
                    <div style="border:2px dashed rgba(255,255,255,0.1); border-radius:16px; padding:20px; text-align:center; cursor:pointer; transition:all 0.3s; position:relative; overflow:hidden;" 
                        ...
                        <div id="upload-placeholder" style="{{ $project->image_url ? 'display:none;' : '' }}">
                        ...
                        <img id="image-preview" src="{{ $project->image }}" style="{{ $project->image_url ? 'display:block;' : 'display:none;' }} width:100%; border-radius:10px; object-fit:cover; aspect-ratio:4/3;" alt="Preview">
                        
                        <div style="position:absolute; bottom:10px; right:10px; background:rgba(0,0,0,0.6); padding:6px; border-radius:8px; backdrop-filter:blur(4px); border:1px solid rgba(255,255,255,0.1);">
                            <svg viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" style="width:14px;height:14px;"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-card" style="padding:24px;">
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px;">
                        ...
                        <label style="position:relative; display:inline-block; width:44px; height:24px;">
                            <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $project->is_featured) ? 'checked' : '' }} style="opacity:0; width:0; height:0;" id="featured-toggle">
